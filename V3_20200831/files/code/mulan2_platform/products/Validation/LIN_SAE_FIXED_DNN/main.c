...
int main(void)
{
    ...
    SAE_apinfo = 1u << 4; /* application requiers attention from Master */
...
void mlu_SAE_MessageReceived (ml_MessageID midx)
{
    if (msg_SUB1 == midx) {       /*---------------------------- RX, 8 bytes */
        if (   (LinFrameDataBuffer[0] == 'M')
            ...
            /* Placeholder for the Frame Handler ... */

        }
    }
    else {  /* --------------- Message ID is not recognized by application */
    }
}
...
{
    if (msg_PUB1 == midx) {  /*--------------------- TX, 2 bytes */
        /* 
         * LinFrameDataBuffer[0] shall not be used, since
         * it will be overwritten by SAE J2602 Status Byte
         */
        LinFrameDataBuffer[1] = 0xFF;  /* .. some data; unused bits SHALL be
                                        * set to recessive (1) level
                                        */
        ml_SAE_DataReady(SAE_apinfo);
    }
    else {
        ml_DiscardFrame(); /* Message ID is not recognized by application */
    }
}
