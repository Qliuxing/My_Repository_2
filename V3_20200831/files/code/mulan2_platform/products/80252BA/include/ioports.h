...
FARPORTW (ZC_BLANK_MASK,            0x2056)
FARPORTW (ZC_BLANK_PEND,            0x2058)
FARPORTW (ZC_BLANK_JMP_VECTOR,      0x205A)


/* ----------------------------------------------------------------------------
 *  Product specific USER ports
 */
PORTB(EXTIO_IN,                     0x281A)                                     /*! input register for the external IOs */
...
PORTB(EXTIO_OPEN_DRAIN,             0x281B)                                     /*! open drain control bits of the external IOs */
...
PORTB(DRV_CTRL,                     0x281C)                                     /*! control register for the motor driver */
    #define PWM_START               (1u << 0)                                   /*! start the clock of the PWM module */
    #define CURRENT_REG_EN          (1u << 1)                                   /*! enable the current regulation */
    #define STATE_TIMER_START       (1u << 2)
    #define ZC_TIMER_START          (1u << 3)
    #define ZC_EDGEDET_START        (1u << 4)
    #define ZC_MASKED_ENABLE        (1u << 5)
    #define NEXT_STATE_OPTION       (1u << 6)
    #define NEXT_CURREG_MASK_OPTION (1u << 7)
...
PORTB(DRV_CONFIG,                   0x281E)                                     /*! configuration register of the driver */
    ...
    #define MOTOR_CONTROL_CLOCK_0   (1u << 3)
    #define MOTOR_CONTROL_CLOCK_1   (1u << 4)
    #define MOTOR_CONTROL_CLOCK_2   (1u << 5)
    #define ROTOR_POSITION_0        (1u << 6)
    #define ROTOR_POSITION_1        (1u << 7)

PORTB(DEADTIME_VDS_CONFIG,          0x281F)                                     /*! configure the deadtime for the driver and the VDS detector */
...
PORTW (ZC_CONFIG_STATUS,            0x2820)
    #define ZC_MASK_SELECT_0        (1u << 0)
    #define ZC_MASK_SELECT_1        (1u << 1)
    #define ZC_MASK_HS_LSB          (1u << 2)
    #define ZC_MASK_RIS_FALLB       (1u << 3)
    #define ZC_MASK_TIME_0          (1u << 4)
    #define ZC_MASK_TIME_1          (1u << 5)
    #define ZC_MASK_TIME_2          (1u << 6)
    #define ZC_MASK_TIME_3          (1u << 7)
    #define ZC_INPUT_SELECT_0       (1u << 8)
    #define ZC_INPUT_SELECT_1       (1u << 9)
    #define ZC_RIS_FALLB            (1u << 10)
    #define ZC_NEXT_STATE_OPTION    (1u << 11)
    #define ZC_BLANKED              (1u << 12)      /* read-only */
    #define ZC_EDGE                 (1u << 13)      /* read-only */

PORTW(NEXT_MOTOR_STATE,             0x2822)                                     /*! next motor state register */
    #define NEXT_MOTOR_STATEA_0     (1u << 0)                                   /*! next motor state, bit 0 */
    #define NEXT_MOTOR_STATEA_1     (1u << 1)                                   /*! next motor state, bit 1 */
    #define NEXT_MOTOR_STATEA_2     (1u << 2)                                   /*! next motor state, bit 2 */
    #define NEXT_MOTOR_STATEA_3     (1u << 3)                                   /*! next motor state, bit 3 */
    #define NEXT_MOTOR_STATEA_4     (1u << 4)
    #define NEXT_MOTOR_STATEB_0     (1u << 5)
    #define NEXT_MOTOR_STATEB_1     (1u << 6)
    #define NEXT_MOTOR_STATEB_2     (1u << 7)
    #define NEXT_MOTOR_STATEB_3     (1u << 8)
    #define NEXT_MOTOR_STATEB_4     (1u << 9)
    #define NEXT_MOTOR_STATEC_0     (1u << 10)
    #define NEXT_MOTOR_STATEC_1     (1u << 11)
    #define NEXT_MOTOR_STATEC_2     (1u << 12)
    #define NEXT_MOTOR_STATEC_3     (1u << 13)
    #define NEXT_MOTOR_STATEC_4     (1u << 14)

PORTB (PWMA_COMP,                   0x2824)                                     /*! PWM A duty cycle register */
PORTB (PWMB_COMP,                   0x2825)
PORTB (PWMC_COMP,                   0x2826)
PORTB (PWMI_COMP,                   0x2827)                                     /*! PWM I duty cycle register */

PORTW (STATE_TIME,                  0x2828)
PORTW (STATE_TIME_COMP,             0x282A)
PORTW (ZC_BLANK_TIME_COMP,          0x282C)
PORTW (ZC_PERIOD,                   0x282E)

PORTB(CUR_REG_CTRL,                 0x2830)                                     /*! current regulation control register */
    #define NEXT_CURREG_MASK_SEL_0  (1u << 0)
    #define NEXT_CURREG_MASK_SEL_1  (1u << 1)
...
PORTB(CUR_REG_DAC,                  0x2831)                                     /*! current regulation DAC value */
...
PORTW(SPI_DATA,                     0x283E)                                     /*! SPI data register */
...
FARPORTB (PWM_CAPTURE,              0x284F)

FARPORTW (OTP_SPEC_TEST,            0x2850)
