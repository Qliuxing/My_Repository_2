...
/* -------------------------------------------------------------------
 * Enable External Interrupt 5 (priority is fixed to 1 in HW)
 *
 * MASK[1], PEND[1]
 */
#define EXT5_INT_ENABLE()                   \
    do {                                    \
        PEND  = (1u << 1);                  \
        MASK |= (1u << 1);                  \
    } while (0)
...
/* -------------------------------------------------------------------
 * Disable (mask) External Interrupt 5
 */
#define EXT5_INT_DISABLE()      \
    (MASK &= ~(1u << 1))


/* -------------------------------------------------------------------
 * Enable External Interrupt 6 (priority is fixed to 2 in HW)
 *
 * MASK[3], PEND[3]
 */
#define EXT6_INT_ENABLE()                   \
    do {                                    \
        PEND  = (1u << 3);                  \
        MASK |= (1u << 3);                  \
    } while (0)
...
/* -------------------------------------------------------------------
 * Disable (mask) External Interrupt 6
 */
#define EXT6_INT_DISABLE()      \
    (MASK &= ~(1u << 3))


/* -------------------------------------------------------------------
 * Enable External Interrupt 7 (priority is fixed to 5 in HW)
 *
 * MASK[4], PEND[4]
 */
#define EXT7_INT_ENABLE()                   \
    do {                                    \
        PEND  = (1u << 4);                  \
        MASK |= (1u << 4);                  \
    } while (0)
...
/* -------------------------------------------------------------------
 * Disable (mask) External Interrupt 7
 */
#define EXT7_INT_DISABLE()      \
    (MASK &= ~(1u << 4))
