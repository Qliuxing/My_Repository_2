...
static const ml_uint8 initialNAD = MLX_INITIAL_NAD;
...
#if (LIN_CFG_ASSIGN_NAD == 1)        
        /* Check if it's a ASSIGN NAD (SID = 0xB0)
         * This is checked indpendently from the rest of service requests,
         * since NAD shall be initial NAD || wildcard (0x7F)
         */
        if (((LinFrameDataBuffer[0] == initialNAD) || (LinFrameDataBuffer[0] == 0x7F))
           && (LinFrameDataBuffer[1] == 0x06)
           && (LinFrameDataBuffer[2] == 0xB0))  {

           if (  
                 /* Supplier ID or wilcard */                    
                 ...
                 (    ((LinFrameDataBuffer[5] == ml_ProductID[2]) && (LinFrameDataBuffer[6] == ml_ProductID[3]))
                   || ((LinFrameDataBuffer[5] == 0xFF)            && (LinFrameDataBuffer[6] == 0xFF))
                 )
              )
           {
               ml_Disconnect();             /* should be in disconnected state for executing ml_SAE_SetConfiguration .. */
               ml_SAE_SetConfiguration(LinFrameDataBuffer[7] /* NAD */, NUM_MSG);
               ml_Connect();                /* .. reconnect */

               pendingSlaveResponse = respPositiveAssignNAD;
           }
           else {
           }
        }
    ...
    if (midx == R_DIA) {    /*---- Diagnostic SRF  (assigned to FID=0x3D) --*/
        ...
        switch (pendingSlaveResponse) {
            ...
            case respPositiveAssignNAD:
                ...
                LinFrameDataBuffer[0] = initialNAD;
                LinFrameDataBuffer[1] = 0x01; /* PCI */
                LinFrameDataBuffer[2] = 0xF0; /* RSID = 0xB0+0x40 */
