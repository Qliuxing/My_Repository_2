...
/* WD_T setting for having 256us timeout */
#define     WDT_WD_BIST_VALUE       0x01
...
/*static*/  uint8 su_march_fail      __attribute__((section(".noinit")));
/* ------------------------------------------------------------------------- */
...
#define EE_ACTIVATE 	0x0001
#define M_NOMARCH 		0x01
#define M_NOWDBIST 		0x02
...
void _low_level_init (void)
{
  ...
  /* RESERVED |= EE_ACTIVE; */
  *(unsigned int *)(0x2002) = (*(unsigned int*)(0x2002))| EE_ACTIVATE;
  ...
  /* --- Trimming --------------------------------------------------------- */
    ...
    /* if(Line-Up == 0) then mask IN2GAIN */
    ANA_OUTA = ((*(unsigned char*)(0x1003)) & M_LINE_UP) ? (*(unsigned int*)(0x1000)) : (*(unsigned int*)(0x1000)) & ~M_IN2GAIN; /* eeANAOUTA */
    ANA_OUTC = (*(unsigned char*)(0x1017)) & ~M_FREE1_7; /* eeAna_OutC_L */

    PATCH0_I =  (*(unsigned int*)(0x107A));
    PATCH0_A =  (*(unsigned int*)(0x1078));

    CONTROL &= ~(OUTA_WE | OUTC_WE); /* Do not &= ~OUTB_WE, as still needed by outputstage driver */
    ...
    if ( WD_BOOT_CHECK() && _wd_bist_is_active() ) {
        ...
        if(0 == ((*(unsigned int*)(0x1052)) & M_NOMARCH)) do_memory_test();
        /* _low_level_bists(); */ /* other BISTs */
        if(0 == ((*(unsigned int*)(0x1052)) & M_NOWDBIST)) _wd_bist(); /* wd_bist should be last bist */
    }
} 
...
void _wd_bist (void)
{
    ...
    /* Start the watchdog */
    WD_INIT( WD_MODE_INTELLIGENT, WD_DIV_8, WDT_WD_BIST_VALUE); /* Time_out = 32us */
    DELAY_US(64); /* wait for WD triggering */
