...
typedef enum {
    ...
    respBaudrateResponse   /* answer with current baudrate value */
} SlaveResponseType;
            ...
            if ( ((supp_id == MLX_SUPPLIER_ID) || (supp_id == 0x7FFF))
              && ((func_id == MLX_FUNCTION_ID) || (func_id == 0xFFFF)) ) {
                ...
                switch (Q_ID) { /* Question */
                    ...
                    case 0x07:
                        pendingSlaveResponse = respBaudrateResponse;
                        break;
...
void mlu_DataRequest (ml_MessageID midx)
{
    ...
    if (midx == ML_SRF_INDEX) {       /*---- Diagnostic SRF  (assigned to FID=0x3D) --*/
        ...
        switch (pendingSlaveResponse) {
            ...
            case respBaudrateResponse:
                currentBaudrate = ml_GetBaudRate();

                LinFrameDataBuffer[0] = MLX_INITIAL_NAD;
                LinFrameDataBuffer[1] = 0x03;  /* PCI */
                LinFrameDataBuffer[2] = 0x7F;  /* RSID = 0x7F  */
                LinFrameDataBuffer[3] = 0xBA;  /* BA */
                LinFrameDataBuffer[4] = 0x4D;  /* 4D */
                LinFrameDataBuffer[5] = (uint8_t)((currentBaudrate & 0xFF00) >> 8); /* High byte of baudrate value */
                LinFrameDataBuffer[6] = (uint8_t)(currentBaudrate & 0x00FF);        /* Low byte of baudrate value  */
                LinFrameDataBuffer[7] = 0x00;  /* zero value */
                ml_DataReady(ML_END_OF_TX_DISABLED);     /* Signal to MLX4 */
                break;
